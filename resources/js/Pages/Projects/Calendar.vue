                            ...
                            <div class="grid grid-cols-7 flex-1" style="grid-template-rows: repeat(6, 1fr);">
                                ...
                                >
                                    ...
                                    <div class="space-y-1.5 flex-1">
                                        ...
                                        <div
                                            v-for="gEvent in getGoogleEventsForDay(day.date)"
                                            :key="'g-' + gEvent.id"
                                            class="text-xs p-2 rounded-xl cursor-pointer bg-green-50 border border-green-200 hover:bg-green-100 transition-all duration-200"
                                            :title="gEvent.title"
                                        >
                                            <div class="flex items-center space-x-1.5">
                                                <svg class="w-3 h-3 flex-shrink-0 text-green-600" viewBox="0 0 24 24" fill="currentColor">
                                                    <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/>
                                                    <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
                                                    <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/>
                                                    <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/>
                                                </svg>
                                                <span class="truncate font-medium text-green-800">{{ gEvent.title }}</span>
                                            </div>
                                            <div v-if="!gEvent.all_day" class="mt-1 text-green-600 ml-4">
                                                {{ moment(gEvent.start).format('HH:mm') }}
                                            </div>
                                        </div>
                                    </div>
                                </div>
...
export default {
    ...
    methods: {
        ...
        async checkGoogleConnection() {
            try {
                const res = await axios.get(this.route('google.calendar.status'))
                this.googleCalendarConnected = res.data.connected
                if (res.data.connected) {
                    this.fetchGoogleEvents()
                }
            } catch (e) {
                // not connected or error, silently skip
            }
        },
        async fetchGoogleEvents() {
            ...
            try {
                const start = this.moment(this.currentDate).startOf('month').subtract(1, 'week').format('YYYY-MM-DD')
                const end = this.moment(this.currentDate).endOf('month').add(1, 'week').format('YYYY-MM-DD')
                const res = await axios.get(this.route('google.calendar.events'), { params: { start, end } })
                this.googleEvents = res.data.events || []
            } catch (e) {
                this.googleEvents = []
            } finally {
        ...
        getGoogleEventsForDay(date) {
            if (!this.showGoogleEvents || !this.googleCalendarConnected) return []
            const day = this.moment(date).format('YYYY-MM-DD')
            return this.googleEvents.filter(event => {
                const eventDate = this.moment(event.start).format('YYYY-MM-DD')
    ...
    watch: {
        selectedDate() {
            if (this.googleCalendarConnected) {
                this.fetchGoogleEvents()
            }
        },
        currentDate() {
