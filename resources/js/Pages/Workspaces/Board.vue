<template>
  <div class="h-full" :class="{'right_menu_enable': show_right_menu}">
      <Head :title="$t(title)" />
      <workspace-view-menu :workspace="workspace" @filter-toggle="open_filter = !open_filter" @menu-toggle="show_right_menu = !show_right_menu" @fClear="reset()" @exportTasks="handleExportTasks" @showArchived="handleShowArchived" :filters="filters" view="board" />
      <board-filter :workspace="workspace" @board-filter="open_filter = false" :filters="filters" v-if="open_filter" @do-filter="doFilter" options="user,due,label,project"  />
  ...
  layout: Layout,
    ...
    methods: {
      ...
      cardSwitchToggle(element, e){
        ...
        handleExportTasks(){
            // Export all tasks from workspace
            const projectIds = this.workspaceProjects.map(p => p.id).join(',');
            if (projectIds) {
                // For now, export from first project or create a workspace export route
                window.location.href = this.route('project.csv.export', this.workspaceProjects[0]?.id || this.workspace.id);
            } else {
                console.warn('No projects found in workspace');
            }
        },
        handleShowArchived(type){
            // Show archived items modal or navigate to archived view
            if (type === 'tasks') {
                // Navigate to archived tasks view or show modal
                this.$inertia.get(this.route('workspace.view.board', this.workspace.slug || this.workspace.id), { archived: 'tasks' }, { preserveState: true });
            } else if (type === 'boards') {
                // Navigate to archived boards view or show modal
