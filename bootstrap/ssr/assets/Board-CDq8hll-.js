...
import { resolveComponent, mergeProps, withCtx, createVNode, openBlock, createBlock, createTextVNode, toDisplayString, createCommentVNode, Fragment, renderList, withDirectives, vModelText, vShow, useSSRContext } from "vue";
import { ssrRenderAttrs, ssrRenderComponent, ssrRenderClass, ssrRenderList, ssrInterpolate, ssrRenderAttr, ssrRenderStyle, ssrIncludeBooleanAttr, ssrLooseContain, ssrLooseEqual } from "vue/server-renderer";
...
const _sfc_main = {
  ...
  watch: {
    form: {
      ...
      handler: throttle(function() {
        this.$inertia.get(this.route("workspace.view.board", this.workspace.slug || this.workspace.id), pickBy(this.form), { preserveState: true });
      }, 150)
    }
  },
  created() {
  ...
  methods: {
    ...
    handleExportTasks() {
      var _a;
      const projectIds = this.workspaceProjects.map((p) => p.id).join(",");
      if (projectIds) {
        window.location.href = this.route("project.csv.export", ((_a = this.workspaceProjects[0]) == null ? void 0 : _a.id) || this.workspace.id);
      } else {
        console.warn("No projects found in workspace");
      }
    },
    handleShowArchived(type) {
      if (type === "tasks") {
        this.$inertia.get(this.route("workspace.view.board", this.workspace.slug || this.workspace.id), { archived: "tasks" }, { preserveState: true });
      } else if (type === "boards") {
        this.$inertia.get(this.route("workspace.view.board", this.workspace.slug || this.workspace.id), { archived: "boards" }, { preserveState: true });
      }
    },
...
function _sfc_ssrRender(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  ...
  _push(ssrRenderComponent(_component_workspace_view_menu, {
    ...
    onExportTasks: $options.handleExportTasks,
    onShowArchived: $options.handleShowArchived,
  ...
  if ($data.open_filter) {
    _push(ssrRenderComponent(_component_board_filter, {
      ...
      options: "user,due,label,project"
    }, null, _parent));
  } else {
    _push(`<!---->`);
  }
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Pages/Workspaces/Board.vue");
